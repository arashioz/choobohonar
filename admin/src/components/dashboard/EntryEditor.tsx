...
const copy: Record<CmsKind, { title: string; singular: string }> = {
  product: { title: "محصولات", singular: "محصول" }, material: { title: "متریال‌ها", singular: "متریال" }, project: { title: "پروژه‌ها", singular: "پروژه" }, collection: { title: "کالکشن‌ها", singular: "کالکشن" }, story: { title: "ویدیوهای عمودی", singular: "ویدیو" }, article: { title: "مقالات", singular: "مقاله" }, page: { title: "صفحات", singular: "صفحه" },
};
  ...
  async function upload(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        setEntry((current) => kind === "story"
          ? { ...current, data: { ...(current.data || {}), video: url } }
          : { ...current, images: [...(current.images || []), url] });
        setDirty(true);
...
function MediaList({ images, onChange, videoOnly = false }: { images: string[]; onChange: (images: string[]) => void; videoOnly?: boolean }) {
  ...
  return <div className="mt-4 space-y-2">{images.map((image, index) => <div key={`${image}-${index}`} className="rounded-xl border border-forest/10 bg-[#faf8f5] p-2"><div className="flex items-start gap-2">{videoOnly ? <video src={image} preload="metadata" muted className="h-16 w-16 shrink-0 rounded-lg bg-[#e8e2d9] object-cover" /> : <span className="relative h-16 w-16 shrink-0 rounded-lg bg-[#e8e2d9]" style={{ backgroundImage: `url(${image})`, backgroundSize: "cover", backgroundPosition: "center" }}>{index === 0 && <span className="absolute -right-1 -top-1 rounded-md bg-forest px-1.5 py-0.5 text-[7px] text-paper">اصلی</span>}</span>}<div className="min-w-0 flex-1"><a href={image} target="_blank" rel="noopener noreferrer" className="block break-all text-[9px] leading-4 text-brick underline-offset-2 hover:underline" dir="ltr">{image}</a><span className="mt-1 block text-[9px] text-forest/35">{videoOnly ? "ویدیوی ذخیره‌شده" : "لینک تصویر ذخیره‌شده"}</span></div><div className="flex shrink-0 items-center">{!videoOnly && <><button type="button" disabled={index === 0} onClick={() => move(index, -1)} className="flex h-7 w-6 items-center justify-center text-forest/30 disabled:opacity-20" title="انتقال به بالا">↑</button><button type="button" disabled={index === images.length - 1} onClick={() => move(index, 1)} className="flex h-7 w-6 items-center justify-center text-forest/30 disabled:opacity-20" title="انتقال به پایین">↓</button></>}<button type="button" onClick={() => onChange(images.filter((_, i) => i !== index))} className="flex h-7 w-7 items-center justify-center rounded-lg text-forest/30 hover:bg-brick/[0.05] hover:text-brick" title="حذف">×</button></div></div></div>)}<div className="flex gap-2 pt-1"><input value={url} onChange={(e) => setUrl(e.target.value)} className={`${inputClass} min-w-0`} placeholder={videoOnly ? "یا آدرس ویدیو را وارد کنید" : "یا آدرس تصویر را وارد کنید"} dir="ltr"/><button type="button" onClick={() => { if (url.trim()) { onChange(videoOnly ? [url.trim()] : [...images, url.trim()]); setUrl(""); } }} className="shrink-0 rounded-xl border border-forest/10 px-3 text-[10px] text-forest/55">افزودن</button></div></div>;
}
