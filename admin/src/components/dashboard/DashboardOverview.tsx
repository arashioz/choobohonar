...
function kindLabel(kind: CmsEntry["kind"]) { return ({ article: "مقاله", product: "محصول", material: "متریال", project: "پروژه", collection: "کالکشن", page: "صفحه" } as const)[kind] || "محتوا"; }
function entryHref(item: CmsEntry) { if (item.kind === "article") return `/admin/articles/${item._id}`; const path = ({ product: "products", material: "materials", project: "projects", collection: "collections" } as const)[item.kind as "product" | "material" | "project" | "collection"]; return path ? `/admin/manage/${path}/${item._id}` : "/admin"; }
