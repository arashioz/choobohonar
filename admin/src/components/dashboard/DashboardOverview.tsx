...
type Summary = Record<"article" | "product" | "material" | "project" | "collection" | "story", { total: number; published: number; draft: number; items: CmsEntry[] }>;
const empty = { total: 0, published: 0, draft: 0, items: [] };

export default function DashboardOverview() {
  const [summary, setSummary] = useState<Summary>({ article: empty, product: empty, material: empty, project: empty, collection: empty, story: empty });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    Promise.all((["article", "product", "material", "project", "collection", "story"] as const).map(async (kind) => {
  ...
  return <main className="min-h-screen bg-[#f6f3ee]"><div className="mx-auto max-w-[1380px] px-5 py-7 sm:px-8 md:py-9 lg:px-10">
    ...
    <div className="mt-5 grid gap-5 lg:grid-cols-12">
      <div className="space-y-5 lg:col-span-8">
        <section className="rounded-2xl border border-forest/10 bg-white/75 p-5 sm:p-6"><div className="flex items-center justify-between border-b border-forest/[0.07] pb-4"><div><h2 className="text-sm font-medium text-forest">محتوا و تحریریه</h2><p className="mt-1 text-[10px] text-forest/35">مدیریت مقاله مستقل از ابزار تولید محتوا</p></div><Link href="/admin/articles" className="text-[10px] font-medium text-brick">مشاهده مقالات ←</Link></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3"><ActionCard href="/admin/articles" number={summary.article.total} eyebrow="EDITORIAL CMS" title="مقالات" description="ایجاد، تکمیل، ویرایش و انتشار کلاسیک" icon="A"/><ActionCard href="/admin/content" eyebrow="AI WORKSPACE" title="چوب‌نویس" description="دستیار چندمنظوره برای ایده، مقاله و متن محصول" icon="✦" accent/><ActionCard href="/admin/seo" eyebrow="SEO DASHBOARD" title="داشبورد سئو" description="وضعیت ایندکس، صفحات و برنامه بهینه‌سازی محتوا" icon="S"/></div></section>

        <section className="rounded-2xl border border-forest/10 bg-white/75 p-5 sm:p-6"><div className="flex items-center justify-between border-b border-forest/[0.07] pb-4"><div><h2 className="text-sm font-medium text-forest">کاتالوگ و آثار</h2><p className="mt-1 text-[10px] text-forest/35">داده‌های واقعی فروش، تولید و نمایش سایت</p></div><Link href="/admin/manage" className="text-[10px] font-medium text-brick">مرکز مدیریت ←</Link></div><div className="mt-2 divide-y divide-forest/[0.07]"><CatalogRow href="/admin/manage/products" label="محصولات" description="قیمت، موجودی و مشخصات" value={summary.product.total}/><CatalogRow href="/admin/manage/materials" label="متریال‌ها" description="کتابخانه و موجودی مواد" value={summary.material.total}/><CatalogRow href="/admin/manage/projects" label="پروژه‌ها" description="اطلاعات و گالری اجرا" value={summary.project.total}/><CatalogRow href="/admin/manage/collections" label="کالکشن‌ها" description="مجموعه و اتصال محصولات" value={summary.collection.total}/><CatalogRow href="/admin/manage/stories" label="ویدیوهای عمودی" description="بخش ویدیویی صفحه محصولات" value={summary.story.total}/></div></section>
      </div>
...
function kindLabel(kind: CmsEntry["kind"]) { return ({ article: "مقاله", product: "محصول", material: "متریال", project: "پروژه", collection: "کالکشن", story: "ویدیو", page: "صفحه" } as const)[kind] || "محتوا"; }
function entryHref(item: CmsEntry) { if (item.kind === "article") return `/admin/articles/${item._id}`; const path = ({ product: "products", material: "materials", project: "projects", collection: "collections", story: "stories" } as const)[item.kind as "product" | "material" | "project" | "collection" | "story"]; return path ? `/admin/manage/${path}/${item._id}` : "/admin"; }
