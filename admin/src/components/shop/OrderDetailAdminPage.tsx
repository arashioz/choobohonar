      ...
      <main className="relative z-10 mx-auto max-w-4xl space-y-6 px-5 py-8 sm:px-8">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <section className="rounded-2xl border border-forest/10 bg-white/80 p-5 text-sm">
            ...
            {typeof order.shipping.lat === "number" && typeof order.shipping.lng === "number" ? <><p className="mt-2 text-xs text-forest/40" dir="ltr">{order.shipping.lat.toFixed(5)}, {order.shipping.lng.toFixed(5)}</p><div className="mt-3 overflow-hidden rounded-xl border border-forest/10"><iframe title="موقعیت مشتری روی نقشه" className="h-56 w-full border-0" loading="lazy" src={`https://www.openstreetmap.org/export/embed.html?bbox=${order.shipping.lng - 0.02}%2C${order.shipping.lat - 0.015}%2C${order.shipping.lng + 0.02}%2C${order.shipping.lat + 0.015}&layer=mapnik&marker=${order.shipping.lat}%2C${order.shipping.lng}`} /></div><a className="mt-2 inline-block text-xs text-brick hover:underline" href={`https://www.openstreetmap.org/?mlat=${order.shipping.lat}&mlon=${order.shipping.lng}#map=16/${order.shipping.lat}/${order.shipping.lng}`} target="_blank" rel="noreferrer">باز کردن نقشه در صفحه جدید</a>{order.shipping.mapNote ? <p className="mt-2 text-xs text-forest/50">توضیح مسیر: {order.shipping.mapNote}</p> : null}</> : <p className="mt-2 text-xs text-forest/40">موقعیت نقشه ثبت نشده است.</p>}
          </section>
        </div>
