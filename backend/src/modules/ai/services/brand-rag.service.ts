...
import {
  ...
  BrandDocument,
  BrandCategory,
} from '../schemas/brand-chunk.schema';
...
export class BrandRagService implements OnModuleInit {
  ...
  async seedBrandVectorStore() {
    ...
    for (const item of brandKnowledge) {
      const vector = await this.localEmbeddingService.generateEmbedding(
        item.content,
      );
    ...
    console.log(
      `✅ [ChooboHonar RAG] Successfully generated 384d Embeddings for ${this.embeddedStore.length} Brandbook Chunks.`,
    );
  }
  ...
  async searchSimilar(query: string, limit = 3) {
    const queryVector =
      await this.localEmbeddingService.generateEmbedding(query);
