...
function rulesForCmsCollection(collection: CmsCollection): MatchRule[] {
  ...
  return names.map((name) => ({
    name,
    series: String(data.seriesName || data.series || name).trim(),
  }));
}
...
async function main() {
  ...
  const NamedCollectionModel = mongoose.model<NamedCollection>(
    'Collection',
    new Schema({}, { strict: false }),
  );
  ...
  const CmsCollectionModel = mongoose.model<CmsCollection>(
    'CmsSyncCollection',
    new Schema({}, { strict: false }),
    'cms_entries',
  );
  ...
  for (const product of products) {
    ...
    if (
      found.length > 1 &&
      normalize(found[1].name).length === normalize(best.name).length &&
      normalize(found[1].name) !== normalize(best.name)
    )
      continue;
  ...
  for (const collection of cmsCollections) {
    const matchedSlugs = products
      .filter(
        (product) =>
          matches(product, rulesForCmsCollection(collection)).length > 0,
      )
      .map((product) => product.slug);
    const existing = Array.isArray(collection.data?.productSlugs)
      ? collection.data.productSlugs
      : [];
    ...
    cmsOperations.push({
      updateOne: {
        ...
        update: {
          $set: { 'data.productSlugs': next, 'data.productIds': next },
        },
      },
    });
    collectionsUpdated.push({
