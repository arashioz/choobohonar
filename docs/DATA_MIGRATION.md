...
برای انتقال لینک‌های تصاویر remote به فایل‌های local و ثبت آدرس عمومی در MongoDB، یک‌بار روی سرور اجرا کنید:

```bash
docker compose exec backend npm run migrate:media
```

این دستور تصاویر محصولات و تمام تصاویر ثبت‌شده در CMS را در `backend/uploads/migrated` ذخیره می‌کند و لینک‌ها را به `/uploads/migrated/<hash>.<ext>` تبدیل می‌کند. اجرای دوباره امن است؛ فایل‌های موجود دوباره دانلود نمی‌شوند. برای مشاهده‌ی تعداد لینک‌ها بدون تغییر دیتابیس از `npm run migrate:media -- --dry-run` استفاده کنید.
