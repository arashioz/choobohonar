/** Category playbooks for storefront variants. Keep in sync with backend/src/modules/shop/variant-playbook.ts */
...
export function isSizeAttribute(name: string, values: string[] = []) {
  return /^(سایز|اندازه|size)$/i.test(name.trim()) || isSeatAttribute(name, values);
}
...
export function purchaseAttributeLabel(name: string, values: string[] = []) {
  ...
  if (isHeadboardTypeAttribute(name, values)) return "نوع سرتخت";
  if (isHeadboardMaterialAttribute(name)) return "سرتخت";
  return name.trim();
}
