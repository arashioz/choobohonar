...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return { title: `${collection.name} | خانه چوب و هنر`, description: collection.shortDescription };
}
...
export default async function CollectionDetailPage({ params }: PageProps) {
  ...
  if (apiCollection) {
    const products = apiCollection.products || [];
    return (
      <section className="bg-paper pt-32 pb-24 md:pt-40 md:pb-32">
        <Container>
          <nav className="mb-10 flex items-center gap-2 text-sm text-forest/55">
            ...
            <span>/</span>
            <span className="text-forest">{apiCollection.name}</span>
          </nav>

          <div className="grid grid-cols-1 gap-12 border-b border-forest/10 pb-14 lg:grid-cols-[1.15fr_0.85fr] lg:items-end">
            <div>
              <FadeUp as="p" className="eyebrow text-brick">سری {apiCollection.series}</FadeUp>
              <FadeUp as="h1" delay={0.05} className="mt-6 text-balance text-[clamp(2.5rem,7vw,6rem)] font-light leading-[0.95] tracking-tightest text-forest">
                {apiCollection.name}
              </FadeUp>
              <FadeUp as="p" delay={0.15} className="mt-6 max-w-2xl text-lg leading-relaxed text-forest/65">
                {apiCollection.description || apiCollection.excerpt}
              </FadeUp>
            </div>
            {apiCollection.image && (
          ...
          <div className="mt-14 flex items-center justify-between gap-4">
            ...
            <Link href="/products" className="inline-flex items-center gap-2 text-sm text-forest transition-colors hover:text-brick">
              مشاهده همه محصولات <span>←</span>
            </Link>
          </div>
  ...
  return (
    <section className="bg-paper pt-32 pb-24 md:pt-40 md:pb-32">
      <Container>
        ...
        <div className="grid grid-cols-1 gap-12 border-b border-forest/10 pb-14 lg:grid-cols-[1.15fr_0.85fr] lg:items-end">
          <div>
            <FadeUp as="p" className="eyebrow text-brick">{collection.eyebrow}</FadeUp>
            <FadeUp as="h1" delay={0.05} className="mt-6 text-balance text-[clamp(2.5rem,7vw,6rem)] font-light leading-[0.95] tracking-tightest text-forest">
              {collection.name}
            </FadeUp>
            <FadeUp as="p" delay={0.1} className="mt-2 font-display text-xl text-forest/45">{collection.nameEn}</FadeUp>
