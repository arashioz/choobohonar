        ...
        <div className="grid grid-cols-1 gap-x-6 gap-y-12 sm:grid-cols-2 lg:grid-cols-3">
          {projects.map((p, i) => (
            <FadeUp key={p.slug} delay={i * 0.07}>
              <Link href={`/projects/${p.slug}`} className="group block focus-visible:outline-none">
                ...
                <div className="mt-4 flex items-baseline justify-between gap-3">
                  <h3 className="text-xl font-light tracking-tight text-forest transition-colors group-hover:text-brick group-focus-visible:text-brick">{p.title}</h3>
